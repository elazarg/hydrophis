...
class CTranspiler(ast.NodeVisitor):
        ...
        if isinstance(node.func, ast.Name) and node.func.id == 'sizeof':
            if len(node.args) == 1:
                ...
                if isinstance(arg, ast.Name) and arg.id[0].isupper():
                    # Likely a type or struct
                    # For struct types, emit "struct Name"
                    return f"sizeof(struct {arg.id})"
                else:
    ...
    def visit_Expr(self, node: ast.Expr):
        """Handle expression statement."""
        # Check for label: NAME: label
        if isinstance(node.value, ast.Attribute):
            if isinstance(node.value.value, ast.Name) and node.value.attr == 'label':
                label_name = node.value.value.id
                # Labels are not indented and don't have semicolons
                # We could use indent-1, but let's emit at current indent
                self.emit(f"{label_name}:")
                return
